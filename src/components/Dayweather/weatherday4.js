...
const WeatherDay4 = (props) => {
  ...
  React.useEffect(() => {
    ...
    fetch(
      ...
      .then((responseData) => {
        setDayOneMin(responseData.daily[3].temp.min);
        setDayOneMax(responseData.daily[3].temp.max);
        setImageicon(responseData.daily[3].weather[0].icon);
        setForecast(responseData.daily[3].summary);
      })
  ...
  const tooltipHtml =
    '<div>' + threeDaysDate + ':</div><div>' + forecast + '</div>';
